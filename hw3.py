...
def print_location(loc): #Provlima
    ...
    print(name(loc),('('+type(loc)+')').strip(), 'at coordinates', lattitude(loc), longitude(loc))
...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    d = 0
    dist = distance(loc, loc_list[0]) #arxikopoihsh
    for i in range(len(loc_list)):
        if loc_type == type(loc_list[i]):
            if distance(loc, loc_list[i]) < dist and distance(loc, loc_list[i]) != distance(loc, loc): #και η απόσταση του αρχικού σημείου από τον ίδιο τον εαυτό του να μην λαμβάνεται υπόψην
                dist = distance(loc, loc_list[i])
                d = i # επιστροφή του αριθμού i της λιστας όπου βρίσκεται το κοντυνότερο σημείο
        elif loc_type == None:
            if distance(loc, loc_list[i]) < dist and distance(loc, loc_list[i]) != distance(loc, loc): 
                dist = distance(loc, loc_list[i])
                d = i
    return llist[d]
...
def pick_cherries_onebyone(): 
    ...
    cherry_field = ['cherry2']
    print(cherry_field[0])
    cherry_field = ['cherry3']
    print(cherry_field[0])
    cherry_field = ['cherry5']
    print(cherry_field[0])
    cherry_field = ['last cherry']
    print(cherry_field[0])
...
def pick_cherries(field): 
    ...
    while field != None: #field[0] = cherry1, field[1][0] = cherry2, field[1][1][0] = cherry3
        c = field[0]
        print(c)
        field = field[1]
...
def cherry_string(field):
    ...
    return reduce(add, [i for i in flatten(field) if 'cherry' in i]) 
